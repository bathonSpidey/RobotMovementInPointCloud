...
PYBIND11_MODULE(NAME, m) {
	...
	py::class_<rs2::save_to_ply, rs2::filter>(m, "save_to_ply")
		...
		.def("get_mesh", &rs2::save_to_ply::get_mesh, "Get mesh", "p"_a, "color"_a);
	...
	py::class_<rs2::save_to_ply::Mesh>(m, "Mesh")
		.def(pybind11::init<std::vector<rs2::vertex>, std::vector<std::array<uint8_t, 3>>, std::vector<std::array<size_t, 3>>>())
		.def_readwrite("new_vertices", &rs2::save_to_ply::Mesh::new_vertices)
		.def_readwrite("new_colors", &rs2::save_to_ply::Mesh::new_colors)
		.def_readwrite("faces", &rs2::save_to_ply::Mesh::faces);
