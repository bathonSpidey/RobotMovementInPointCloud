// wrap simplify header file for integration with cython
#include "Simplify.h"

namespace Simplify{

  // load points
  void load_points(const int n_points, float* points, int* color, float* granularity, float* t_arr){
    vertices.clear();
    // load verticies
    for (int ii = 0; ii < n_points; ii ++){
      Vertex v;
      v.p.x = points[0 + 3*ii];
      v.p.y = points[1 + 3*ii];
      v.p.z = points[2 + 3*ii];
      v.color[0] = color[0 + 3*ii];
      v.color[1] = color[1 + 3*ii];
      v.color[2] = color[2 + 3*ii];
      if(ii % 3 == 0)
        v.granularity = granularity[ii - 3];
      v.t_arr[ii] = t_arr[ii];
      vertices.push_back(v);

    }
  }

  // load triangles
  void load_triangles(const int n_tri, int* faces){
    triangles.clear();
    for (int ii = 0; ii < n_tri; ii ++){
      Triangle t;
      t.attr = 0;
      t.material = -1;
      t.v[0] = faces[0 + 3*ii];
      t.v[1] = faces[1 + 3*ii];
      t.v[2] = faces[2 + 3*ii];
      triangles.push_back(t);
    }
  }

  // load triangles
  void load_triangles_int64(const int n_tri, int64_t* faces){
    triangles.clear();
    for (int ii = 0; ii < n_tri; ii ++){
      Triangle t;
      t.attr = 0;
      t.material = -1;
      t.v[0] = faces[0 + 3*ii];
      t.v[1] = faces[1 + 3*ii];
      t.v[2] = faces[2 + 3*ii];
      triangles.push_back(t);
    }
  }

  // load triangles from vtk and deal with padding
  int load_triangles_from_vtk(const int n_tri, int* faces){
    triangles.clear();
    for (int ii = 0; ii < n_tri; ii ++){
      Triangle t;
      t.attr = 0;
      t.material = -1;
      if (faces[4*ii] != 3){
        return 1;
      }
      t.v[0] = faces[1 + 4*ii];
      t.v[1] = faces[2 + 4*ii];
      t.v[2] = faces[3 + 4*ii];
      triangles.push_back(t);
    }
    return 0;
  }

  void load_arrays_int32(const int n_points, const int n_tri,
                         float* points, int* faces, int* color, float* granularity, float* t_arr){
    load_points(n_points, points, color, granularity, t_arr);
    load_triangles(n_tri, faces);
  }

  void load_arrays_int64(const int n_points, const int n_tri,
                          float* points, int64_t* faces, int* color, float* granularity, float* t_arr){
    load_points(n_points, points, color, granularity, t_arr);
    load_triangles_int64(n_tri, faces);
  }

  int n_points(){
    return vertices.size();
  }

  int n_triangles(){
    return triangles.size();
  }

  int n_colors(){
    return vertices.size();
  }

  // load triangles
  void load_triangles(const int n_tri, int64_t* faces){
    triangles.clear();
    for (int ii = 0; ii < n_tri; ii ++){
      Triangle t;
      t.attr = 0;
      t.material = -1;
      t.v[0] = faces[0 + 3*ii];
      t.v[1] = faces[1 + 3*ii];
      t.v[2] = faces[2 + 3*ii];
      triangles.push_back(t);
    }
  }

  // populate a contigious array with the points in the vertices vector
  void get_points(float* points){

    // load verticies
    int n_points = vertices.size();
    for (int ii = 0; ii < n_points; ii ++){
      points[0 + 3*ii] = vertices[ii].p.x;
      points[1 + 3*ii] = vertices[ii].p.y;
      points[2 + 3*ii] = vertices[ii].p.z;
    }
  }

  // populate a contigious array with the points in the vertices vector
  void get_triangles(int* tri){

    // load verticies
    int n_tri = triangles.size();
    for (int ii = 0; ii < n_tri; ii ++){
      tri[0 + 3*ii] = triangles[ii].v[0];
      tri[1 + 3*ii] = triangles[ii].v[1];
      tri[2 + 3*ii] = triangles[ii].v[2];
    }
  }

  // populate a contigious array with the points in the vertices vector
  int get_faces_int32(int32_t* tri){

    // load verticies
    int n_tri = triangles.size();
    int jj = 0;
    for (int ii = 0; ii < n_tri; ii ++){
      if (!triangles[ii].deleted){
        tri[0 + 3*jj] = 3;
        tri[1 + 3*jj] = triangles[ii].v[0];
        tri[2 + 3*jj] = triangles[ii].v[1];
        tri[3 + 3*jj] = triangles[ii].v[2];
        jj += 1;
      }
    }
    return jj;
  }

  // populate a contigious array with the points in the vertices
  // vector without the vtk padding
  int get_faces_int32_no_padding(int32_t* tri){

    // load verticies
    int n_tri = triangles.size();
    int jj = 0;
    for (int ii = 0; ii < n_tri; ii ++){
      if (!triangles[ii].deleted){
        tri[0 + 3*jj] = triangles[ii].v[0];
        tri[1 + 3*jj] = triangles[ii].v[1];
        tri[2 + 3*jj] = triangles[ii].v[2];
        jj += 1;
      }
    }
    return jj;
  }

  // populate a contigious array with the points in the vertices vector
  int get_faces_int64(int64_t* tri){

    // load verticies
    int n_tri = triangles.size();
    int jj = 0;
    for (int ii = 0; ii < n_tri; ii ++){
      if (!triangles[ii].deleted){
        tri[0 + 4*jj] = 3;
        tri[1 + 4*jj] = triangles[ii].v[0];
        tri[2 + 4*jj] = triangles[ii].v[1];
        tri[3 + 4*jj] = triangles[ii].v[2];
        jj += 1;
      }
    }
    return jj;
  }

  void get_colors(int* colors){
    int n_colors = vertices.size() * 3;
    for (int i = 0; i < n_colors; i++){
      colors[0 + 3*i] = vertices[i].color[0];
      colors[1 + 3*i] = vertices[i].color[1];
      colors[2 + 3*i] = vertices[i].color[2];
    }
  }
}
