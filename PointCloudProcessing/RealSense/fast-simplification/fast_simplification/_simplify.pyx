...
cdef extern from "wrapper.h" namespace "Simplify":
    void load_arrays_int32(const int, const int, float*, int*, int*, float*, float*)
    void load_arrays_int64(const int, const int, float*, int64_t*, int*, float*, float*)
    ...
    void load_points(const int, float*, int*, float*, float*)


def load_int32(int n_points, int n_faces, float [:, ::1] points, int [:, ::1] faces, int [:, ::1] color, float [:, ::1] granularity, float [:, ::1] t_arr):
    load_arrays_int32(n_points, n_faces, &points[0, 0], &faces[0, 0], &color[0, 0], &granularity[0, 0], &t_arr[0, 0])


def load_int64(
        int n_points, int n_faces, float [:, ::1] points, int64_t [:, ::1] faces, int [:, ::1] color,  float [:, ::1] granularity, float [:, ::1] t_arr
):
    load_arrays_int64(n_points, n_faces, &points[0, 0], &faces[0, 0], &color[0, 0],  &granularity[0, 0], &t_arr[0, 0])
...
def load_from_vtk(int n_points, float [:, ::1] points, int [::1] faces, int n_faces):
    ...
    load_points(n_points, &points[0, 0], NULL, NULL, NULL)
